...
const char* save_print_data_path = "";
...
bool ParseCommandLine(int argc, char* argv[]) {
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg.find(kArgUserSpecific) == 0) {
      ...
      save_print_data_path = arg.substr(arg_len).data();
    }
  }
...
class QueuingHandler : public Handler {
 public:
  QueuingHandler(unsigned long delay, const char* save_print_data_path) :
      Handler(delay, save_print_data_path)  {
